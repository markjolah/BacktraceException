        ...
        if(NOT (${FIXUP_TARGET_TYPE} MATCHES SHARED_LIBRARY OR ${FIXUP_TARGET_TYPE} MATCHES EXECUTABLE) )
            ...
            if(FIXUP_BUILD_TREE_EXPORT)
                ...
                foreach(_lib IN LISTS _libs)
                    if(TARGET ${_lib})
                        list(APPEND FIXUP_BUILD_TARGET_LINK_DIRECTORIES "$<TARGET_FILE_DIR:${_libs}>")
                    elseif(IS_ABSOLUE ${_lib})
                        list(APPEND FIXUP_BUILD_TARGET_LINK_DIRECTORIES ${_lib})
                    endif()
                endforeach()
                message(STATUS "FIXUP_BUILD_TARGET_LINK_DIRECTORIES ${FIXUP_BUILD_TARGET_LINK_DIRECTORIES}")

                set(FIXUP_BUILD_SCRIPT ${CMAKE_BINARY_DIR}/Fixup-Build-${FIXUP_TARGET}.cmake)
                configure_file(${FIXUP_BUILD_SCRIPT_TEMPLATE} ${FIXUP_BUILD_SCRIPT}.gen @ONLY)
                file(GENERATE OUTPUT ${FIXUP_BUILD_SCRIPT} INPUT ${FIXUP_BUILD_SCRIPT}.gen)
                add_custom_target("Fixup-${FIXUP_TARGET}" ALL DEPENDS ${FIXUP_TARGET} COMMAND cmake -P ${FIXUP_BUILD_SCRIPT})
                #add_custom_command(TARGET ${FIXUP_TARGET} POST_BUILD COMMAND cmake -P ${FIXUP_BUILD_SCRIPT})
