...
#                           anouther directory names ${PACKAGE_NAME} with the actual headers
#                         If unset skip setting of traget properties and installing debug headers.
#  PUBLIC_DEBUG_HEADER_DIR - Directory in source for debugging public header includes.
#           Installed as part of Development compnent under Debug configuration.
#           If unset skip setting of traget properties and installing debug headers.
# Multivalue keywords:
...
# SHARED_STATIC_LIB_TARGETS - list of all targets librarties created
#
...
function(add_shared_static_libraries srcs namespace)
    ...
    set(oneValueArgs NAMESPACE LIBTARGET STATIC_LIBTARGET BUILD_SHARED_LIBS BUILD_STATIC_LIBS PUBLIC_HEADER_SRC_DIR PUBLIC_DEBUG_HEADER_SRC_DIR)
    ...
    if(NOT ARG_STATIC_LIBTARGET)
        set(ARG_STATIC_IBTARGET ${PROJECT_NAME}_static)
    endif()
    ...
    if(ARG_BUILD_SHARED_LIBS)
        ...
        #Build static only
        add_library(${ARG_LIBTARGET} STATIC ${srcs})
        add_library(${ARG_NAMESPACE}::${ARG_LIBTARGET} ALIAS ${ARG_LIBTARGET})
        set_target_properties(${ARG_LIBTARGET} PROPERTIES POSITION_INDEPENDENT_CODE ON)
    endif()

    if(NOT ARG_NO_INSTALL)
        install(TARGETS ${SHARED_STATIC_LIB_TARGETS} EXPORT ${PROJECT_NAME}Targets
    ...
    if(ARG_PUBLIC_HEADER_DIR)
        foreach(target ${SHARED_STATIC_LIB_TARGETS})
            if(ARG_PUBLIC_DEBUG_HEADER_DIR)
                target_include_directories(${target} PUBLIC $<BUILD_INTERFACE:${PUBLIC_HEADER_DIR}>
                                                            $<INSTALL_INTERFACE:include>)
            else()
                target_include_directories(${target} PUBLIC $<BUILD_INTERFACE:${PUBLIC_HEADER_DIR}>
                                                            $<BUILD_INTERFACE:$<$<CONFIG:Debug>:${PUBLIC_DEBUG_HEADER_DIR}>>
                                                            $<INSTALL_INTERFACE:include>)
            endif()
        endforeach()
    endif()
    ...
    set(SHARED_STATIC_LIB_TARGETS ${LIB_TARGETS} PARENT_SCOPE ) #Return list of created library targets
endfunction()
